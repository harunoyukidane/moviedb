...
import { getMovieVersion } from '$lib/server/operations';
...
// full getMovie round trip. Never fails the page - a lookup error just means
// no staleness warning this time, not a blocked editor.
export const GET: RequestHandler = async ({ params, request }) => {
  const correlationId = request.headers.get('x-correlation-id') ?? undefined;
  try {
    const version = await getMovieVersion(params.id, { correlationId });
    return json({ version });
  } catch {

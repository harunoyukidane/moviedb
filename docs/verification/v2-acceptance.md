...
| Seed dataset spans enough genres/years to demonstrate both filters | ✅ done | `V3__expand_genre_reference_data.sql` (9 controlled genres); `GENRE_MAP` in `mappings.ts`; `demo/tmdb-movie-ids.txt` (26 ids, 1922–2019); `CatalogueSchemaIntegrationTest`, `importer.test.ts`, `tmdb.test.ts` (manifest/coverage assertions) |

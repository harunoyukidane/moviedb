...
| Movie credits and People rows display photos/fallbacks; unavailable People references degrade gracefully | 🟡 partial (code done, backend deploy pending) | Movie-detail side done and live-verified: `CreditPersonRow`/`CreditSection`, `PersonPhotoFallback`; `CreditPersonRow.test.ts`, `CreditSection.test.ts`, `CreditSection.hydration.test.ts`, `operations.test.ts`. People-list side coded: `Person.photoUrl` in `schema.graphqls`, `PersonListRow.svelte`; `MappersTest.kt`, `CatalogueGraphQlIntegrationTest` (photoUrl case), `PersonListRow.test.ts`, `people/page.server.test.ts` — not yet live-verified because the running Catalogue container predates this schema change (Gradle build unavailable in the authoring sandbox; see frontend.md V2-11) |

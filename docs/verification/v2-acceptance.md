...
| Movie credits and People rows display photos/fallbacks; unavailable People references degrade gracefully | 🟡 partial | Movie-detail side done: `CreditPersonRow`/`CreditSection`, `PersonPhotoFallback`; `CreditPersonRow.test.ts`, `CreditSection.test.ts`, `CreditSection.hydration.test.ts` (zero extra fetches), `operations.test.ts` (single GraphQL request). Live-verified against real TMDB photos. People-list rows (V2-11) not started |

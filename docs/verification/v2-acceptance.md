...
| Credit/view icons have accessible names; movie deletion remains an explicit labeled confirmation | ✅ done | `Icon.svelte`/`IconButton.svelte`/`IconLink.svelte`; edit-icon links to the Credit Editor, delete-icon + `ConfirmDialog` for credit removal; movie/person deletion unchanged (labeled button + `ConfirmDialog`, never icon-only). `IconButton.test.ts`, `IconLink.test.ts`; `journey.spec.ts` updated for the added confirm step. Live-verified: 44×44px target size, confirm-dialog copy, focus return to "Add credit" after removal, correct accessible names throughout |

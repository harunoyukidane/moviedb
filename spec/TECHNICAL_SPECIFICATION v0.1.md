...
Status: proposed baseline for the take-home assessment  
...
This architecture deliberately avoids Kafka, Redis, Elasticsearch, Kubernetes, and cloud object storage. They are not justified by the assessment-sized workload. Extension points are retained where growth would warrant them.
...
### 2.2 Deliberate domain semantics
...
### 2.3 Out of scope unless the assessor says otherwise

- Authentication, authorization, accounts, and multi-tenancy
- Reviews, ratings, watchlists, recommendations, and TV programmes
...
### 2.4 Optional enhancements, only after the MVP is complete
...
- Object-storage implementation of the artwork interface
...
| Deployment | Reproducible local execution via Docker Compose is required; hosting is optional |
...
|---|---|---|
| UI | Svelte 5 + SvelteKit 2 | Required Svelte ecosystem; concise reactive UI, routing, SSR/prerender options |
...
| Artwork | Local volume behind `ArtworkStore` | Zero cloud dependency for local assessment; replaceable with S3/MinIO later |
...
- Movie CRUD and validation
- Movie-credit CRUD and ordering
- Artwork validation, storage, metadata, and download
...
- Person CRUD, validation, and search
...
| Replace artwork | Validate and stage file, commit metadata swap, then best-effort delete old file |
...
Input types for movie/person fields should be explicit rather than reusing output types. Limit is clamped to 1–100. Blank search text is rejected or treated as a normal list consistently.
...
5. Stream to a temporary file, compute SHA-256, then atomically move into the managed volume.
6. Insert/swap artwork metadata in a transaction.
7. Delete the previous file only after the replacement is committed; retry orphan cleanup later if needed.
8. Serve with correct `Content-Type`, `Content-Length`, caching/ETag, and `X-Content-Type-Options: nosniff`.

The application exposes `ArtworkStore` with `put`, `open`, and `delete`. `LocalArtworkStore` is used for the assessment. An S3 implementation could be substituted without changing application use cases.

### Person profile images (hybrid, decision recorded)

Person profile images use a hybrid model rather than either extreme:

- **Import-time provenance URL.** When a person is seeded from TMDB, the `profile_path` is captured and stored. The UI renders this URL for display until a photo is uploaded. This is provenance data captured once at import; the running application has no live runtime dependency on TMDB for it.
- **In-app upload is the only interactive control.** Inside the app, a person's photo is managed by uploading through the same validated `ArtworkStore` path used for movie artwork (magic-byte validation, size limit, server-generated key). There is no in-app "paste a URL" field. Once a photo is uploaded, it takes precedence over any stored TMDB URL.

This keeps the create/amend/delete surface consistent and validated while still giving seeded people sensible imagery. Person-photo upload is sequenced after the core person/movie/credit flows.


The assessor explicitly allowed the candidate to decide whether GraphQL is appropriate for artwork. A conventional multipart HTTP endpoint is therefore selected. It supports streaming and ordinary media caching without depending on the non-core GraphQL multipart upload convention.
...
| Route | Purpose |
|---|---|
| `/` or `/movies` | Paginated catalogue and search entry |
| `/movies/new` | Create movie |
| `/movies/[id]` | Movie details, cast, creators, artwork |
| `/movies/[id]/edit` | Edit movie, artwork, and credits |
| `/people` | Paginated/searchable people directory |
...
- `demo/tmdb-movie-ids.txt` containing roughly 10–15 stable numeric movie IDs
...
| File write succeeds, DB fails | Compensating delete of new file; orphan sweeper is a safety net |
...
- Record Micrometer counters/timers for GraphQL operations, gRPC calls, import outcomes, artwork failures, and missing person references.
...
| E2E | Critical journeys only | create person → create movie → add credit → upload art → search → remove/delete |
...
12. Manage person profile images as a hybrid: TMDB URL as import-time provenance, in-app upload through `ArtworkStore` as the only interactive control.

...
import com.moviecatalogue.people.PeopleServiceApplication
...
/**
 * Proves storage backend selection (V2-02) is genuinely config-driven for people-service,
 * mirroring catalogue-service's ArtworkStoreConfigurationTest.
 */
@Testcontainers
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE)
class PhotoStoreLocalSelectionTest {
...
@Testcontainers
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE)
class PhotoStoreMinioSelectionTest {
    ...
    companion object {
        ...
        private const val BUCKET = "people-config-test-bucket"
...
class PhotoStoreFailFastTest {
    ...
    private fun baseProperties() = arrayOf(
    ...
    // (${PEOPLE_DB_URL:jdbc:postgresql://localhost:5433/people}) would otherwise win over,
    ...
    fun `startup fails clearly when a required MinIO property is missing`() {
        val thrown = catchThrowable {
            SpringApplicationBuilder(PeopleServiceApplication::class.java)
                .web(WebApplicationType.NONE)
                .run(
                    *baseProperties().map { "--$it" }.toTypedArray(),

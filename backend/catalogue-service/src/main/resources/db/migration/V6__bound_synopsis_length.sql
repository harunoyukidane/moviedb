-- V2.2-05: `synopsis` had no length bound at any layer (TEXT, no CHECK), so a
-- single request could store an arbitrarily large document. The app now
-- enforces MovieRules.SYNOPSIS_MAX = 5000 UTF-16 code units; this CHECK is the
-- backstop outside that rule, not a mirror of it.
--
...
ALTER TABLE movie
    ADD CONSTRAINT movie_synopsis_length_check CHECK (char_length(synopsis) <= 5000);

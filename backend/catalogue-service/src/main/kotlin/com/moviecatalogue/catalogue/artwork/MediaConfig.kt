...
import com.moviecatalogue.catalogue.observability.ObjectStorageHealthIndicator
...
/**
 * Wires the shared media components (§10). Storage backend is config-driven
 * (artwork.storage.type=local|minio, V2-02) so Compose can select MinIO while
 * focused local/unit tests keep using the filesystem-backed store.
 */
@Configuration
class MediaConfig {
    ...
    fun localArtworkStore(@Value("\${artwork.storage-path:./data/artwork}") path: String): ArtworkStore =
